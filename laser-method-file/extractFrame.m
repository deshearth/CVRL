...
for i=1:vidNum
    ...
    if ~exist(fullfile(path.processed,'video',filename),'file')
        while hasFrame(vidObj)
            im = readFrame(vidObj);
            if mean(im(:))>10 % get rid of light frames
                continue;
            end
            info.vidth = i;
            info.frameth = k;
...
mk(writePath);
filename = [vidName,'frame',num2str(info.frameth),'.png'];
